...
namespace std
{
	...
	public:
		...
		{
			return hash<int>()( x.first ) ^ hash<int>()( x.second );// 异或
		}
	};
}
...
namespace ss
{
	...
	// Able: 2020.04.10 采用最小生成树MST 来对像素进行逐个分类
	// 1. 边的权重越小，说明两个像素越相似
	...
	public:
		...
		// 从位置x开始，找到第一不等的位置, 链表？
		// 经过多次join后，会出现p值与下标不一致现象，需要通过链表的方式，找到p值与下标一致的，这才是最后的分类
		int find( int x )
		{
			int y = x;
		...
		// 谁的rank 大，就增加谁的size, 并且把小的p设置为大的p
		// 位置p设置为 rank值大的 位置（分类ID), 由于多次合并，会产生链表现象
		void join( int x, int y )
		{
			if ( elements[x].rank > elements[y].rank )
			{
				elements[y].p = x; // 可以理解为分类的ID
	...
	struct edge
	{
		int a; // 顶点a, index (x,y) => y*width + x
		int b; // 顶点b index (x,y) => y*width + x
		double w; // 权重  diff(), 三个通道的颜色差平方之和的平方根
		// 颜色越相近，变化不大，w权重越小
	};
	...
	std::shared_ptr<Universe> segmentGraph( int numVertices, int numEdges, std::vector<edge> &edges, double scale )
	{
		...
		{
			int a = universe->find( pedge.a ); // 找到 该像素点的最后分类ID
			int b = universe->find( pedge.b ); // 找到 该像素点的最后分类ID
			...
			if ( a != b )
			{
				...
				{
					universe->join( a, b );
					a = universe->find( a );// 找到 该像素点的最后分类ID
	...
	std::shared_ptr<Universe> segmentation( const cv::Mat &img, double scale, double sigma, int minSize )
	{
		...
		printf("segmentation(), width=%d, height=%d\n", width, height);
		...
		printf("blurred width:%d, height:%d\n", blurred.cols, blurred.rows);
/*
		cv::Mat testF;
		blurred.convertTo(testF, CV_8UC3, 255);
		int myW = testF.cols;
		int myH = testF.rows;
		for (int row = 0; row < myH;row++)
		{
			for (int col = 0; col < myW; col++)
			{
				testF.at<cv::Vec3b>(row, col)[0] = 255 - blurred.at<cv::Vec3f>(row, col)[0] * 255;
				testF.at<cv::Vec3b>(row, col)[1] = 255 - blurred.at<cv::Vec3f>(row, col)[1] * 255;
				testF.at<cv::Vec3b>(row, col)[2] = 255 - blurred.at<cv::Vec3f>(row, col)[2] * 255;
			}
		}
		cv::imshow("blurred", testF);
*/

		// 一个顶点有相邻的8个顶点，取一半， 为4个方向
		...
		for ( int i = 0; i < num; i++ )
		{
			int a = universe->find( edges[i].a ); // 获取像素点 a的最后分类
			int b = universe->find( edges[i].b ); // 获取像素点 b的最后分类
	...
	void visualize( const cv::Mat &img, std::shared_ptr<Universe> universe )
	{
		...
		for ( int y = 0; y < height; y++ )
		{
			for ( int x = 0; x < width; x++ )
			{
				// universe->find( y*width + x ) 获取像素点 (x, y)的最后分类对应的点
				// 用该点的颜色 填充该分类的所有点的颜色
...
//		cv::waitKey( 1 );
	...
	struct Region
	{
		...
		std::vector<float> colourHist;// 颜色直方图
		std::vector<float> textureHist; // 纹理直方图
	...
	std::map<int, Region> extractRegions( const cv::Mat &img, std::shared_ptr<Universe> universe )
	{
		...
		for ( int y = 0; y < img.rows; y++ )
		{
			for ( int x = 0; x < img.cols; x++ )
			{
				int label = universe->find( y*img.cols + x );// 找到像素点(x,y)的最后分类

				if ( R.find( label ) == R.end() )// 分类在map中还不存在
				{
		...
		{
			// labelRegion.second 就是map中的Region
			// 计算Region的大小， 颜色直方图， 纹理直方图
			// 大小是该类型Region包含的像素点的数量（像素点个数）
	...
	// ss::selectiveSearch(img, 500, 0.8, 50, 20000, 100000, 2.5);
